...
from .sr830_config_ui import Ui_SR830_Config_Window
...
if _SPLIT_LOG:
    sr830_logger = logging.getLogger("sr830")
    sr830_logger.setLevel(logging.DEBUG)
    sr830_fh = logging.FileHandler("sr830.log")
    sr830_formatter = logging.Formatter('%(asctime)s [%(component)s] - %(levelname)s - %(message)s')
    sr830_fh.setFormatter(sr830_formatter)
    sr830_logger.addHandler(sr830_fh)

    sr830_ch = logging.StreamHandler()
    sr830_ch.setFormatter(sr830_formatter)
    sr830_logger.addHandler(sr830_ch)
else:
    sr830_logger = logging.getLogger("autools_setup_main")
...
class SR830Man(QObject):
    """Helper (man) Class for communicate with SR830"""
    ...
    def send_cmd(self, cmd: str, b_query: bool, *args):
        ...
        sr830_logger.debug(f"Sending command <{cmd_str_replaced}>", extra={"component": "SR830"})
        ...
        if self.com_mode == 'rs232':
            with self.ser_lock:
                ...
                if b_query:
                    ...
                    sr830_logger.debug(f"Got answer: <{ans_replaced}>", extra={"component": "SR830"})
                    return ans
                else:
                    return None
        elif self.com_mode == 'gpib':
    ...
    def sensitivity_str2int(sens: str):
        if sens not in SR830Man.SENSITIVITY_LST:
            return None
        else:
            return SR830Man.SENSITIVITY_LST.index(sens)

    @staticmethod
    def sensitivity_int2str(sens_i: int):
        if 0 <= sens_i < len(SR830Man.SENSITIVITY_LST):
            return SR830Man.SENSITIVITY_LST[sens_i]
        else:
            return None
    ...
    def time_constant_str2int(tcons: str):
        if tcons not in SR830Man.TIME_CONSTANT_LST:
            return None
        else:
            return SR830Man.TIME_CONSTANT_LST.index(tcons)

    @staticmethod
    def time_constant_int2str(tcons_i: int):
        if 0 <= tcons_i < len(SR830Man.TIME_CONSTANT_LST):
            return SR830Man.TIME_CONSTANT_LST[tcons_i]
        else:
            return None

    def time_constant(self, b_query: bool = True, i: int = None):
        """
        use SR830Man.time_constant_str2int, and SR830Man.time_constant_int2str
        for the parameter
        """
        if SR830Man._FAKE_DEV:
            return 5
    ...
    def data_sample_rate_str2int(srt: str):
        if srt not in SR830Man.DATA_SAMPLE_RATE_LST:
            return None
        else:
            return SR830Man.DATA_SAMPLE_RATE_LST.index(srt)

    @staticmethod
    def data_sample_rate_int2str(srt_i: int):
        if 0 <= srt_i < len(SR830Man.DATA_SAMPLE_RATE_LST):
            return SR830Man.DATA_SAMPLE_RATE_LST[srt_i]
        else:
            return None
    ...
    def get_points_binary_float(self, dis: int, start_index: int, n_points: int):
        ...
        return SR830Man.non_normalized_float_from_bytes(raw_bytes)
    ...
    def is_command_queue_overflow(self):
        return True if self.standard_event_status(SR830Man.STANDARD_EVENT_STATUS_INP) == 1 else False

    def is_command_output_overflow(self):
        return True if self.standard_event_status(SR830Man.STANDARD_EVENT_STATUS_QRY) == 1 else False

    def is_command_cannot_execute(self):
        return True if self.standard_event_status(SR830Man.STANDARD_EVENT_STATUS_EXE) == 1 else False

    def is_command_illegal(self):
        return True if self.standard_event_status(SR830Man.STANDARD_EVENT_STATUS_CMD) == 1 else False
    ...
    def is_no_scan_in_progress(self):
        return True if self.serial_poll_status(SR830Man.SERIAL_POLL_STATUS_SCN) == 1 else False
    ...
    def is_input_amp_overload(self):
        return True if self.lia_status(SR830Man.LIA_STATUS_RSRV_INPT) == 1 else False

    def is_time_constant_filter_overload(self):
        return True if self.lia_status(SR830Man.LIA_STATUS_FILTR) == 1 else False

    def is_output_overloaded(self):
        return True if self.lia_status(SR830Man.LIA_STATUS_OUTPT) == 1 else False

    def is_reference_unlocked(self):
        return True if self.lia_status(SR830Man.LIA_STATUS_UNLK) == 1 else False

    def is_freq_out_of_range(self):
        return True if self.lia_status(SR830Man.LIA_STATUS_RANGE) == 1 else False

    def is_time_constant_changed(self):
        return True if self.lia_status(SR830Man.LIA_STATUS_TC) == 1 else False

    def is_triggered(self):
        return True if self.lia_status(SR830Man.LIA_STATUS_TRIG) == 1 else False

    def show_config_window(self):
        if self.config_window is None:
            self.config_window = SR830ConfigWindow(self)
...
class SR830ConfigWindow(Ui_SR830_Config_Window):
    """
        SR830 Helper class with configuration window
    """
    def __init__(self, sr830man: SR830Man):
        self.sr830man = sr830man
        self.window = QWidget()
        Ui_SR830_Config_Window.__init__(self)
        ...
        self.comboBox_TimeConstant.currentTextChanged.connect(
            lambda t: self.sr830man.time_constant(False, SR830Man.time_constant_str2int(t)))
        self.comboBox_Sensitivity.currentTextChanged.connect(
            lambda t: self.sr830man.sensitivity(False, SR830Man.sensitivity_str2int(t)))
        self.pushButton_SetHarmonic.clicked.connect(
            lambda: self.sr830man.detection_harmonic(False, self.spinBox_Harmonic.value()))
        self.comboBox_FilterSlope.currentIndexChanged.connect(
            lambda i: self.sr830man.low_pass_filter_slope(False, i))

        self.pushButton_GetX.clicked.connect(
            lambda: self.lineEdit_X.setText(float2str(self.sr830man.get_axis_value(SR830Man.GET_AXIS_VALUE_X)))
        )
        self.pushButton_GetY.clicked.connect(
            lambda: self.lineEdit_Y.setText(float2str(self.sr830man.get_axis_value(SR830Man.GET_AXIS_VALUE_Y)))
        )
        self.pushButton_GetR.clicked.connect(
            lambda: self.lineEdit_R.setText(float2str(self.sr830man.get_axis_value(SR830Man.GET_AXIS_VALUE_R)))
        )
        self.pushButton_GetTheta.clicked.connect(
            lambda: self.lineEdit_Theta.setText(float2str(self.sr830man.get_axis_value(SR830Man.GET_AXIS_VALUE_THETA)))
        )
        ...
        self.pushButton_Autophase.clicked.connect(
            lambda: self.lineEdit_phase.setText(f"{self.sr830man.ref_phase_shift(True):.4f}"))
    ...
    def snap_all_values(self):
        x, y, r, theta, f = self.sr830man.get_parameters_value(SR830Man.GET_PARAMETER_X,
                                                               SR830Man.GET_PARAMETER_Y,
                                                               SR830Man.GET_PARAMETER_R,
                                                               SR830Man.GET_PARAMETER_THETA,
                                                               SR830Man.GET_PARAMETER_REF_FREQ)
    ...
    def ref_src_changed(self):
        if self.radioButton_ref_src_external.isChecked():
            ...
            self.sr830man.ref_source(False, SR830Man.REF_SOURCE_EXTERNAL)
            self.pushButton_SetFrequency.setEnabled(False)
            self.lineEdit_Frequency.setText(f"{self.sr830man.ref_frequency(True): .6f}")

        if self.radioButton_ref_src_internal.isChecked():
            self.radioButton_ref_src_external.setChecked(False)
            self.sr830man.ref_source(False, SR830Man.REF_SOURCE_INTERNAL)
            self.lineEdit_Frequency.setText(f"{self.sr830man.ref_frequency(True): .6f}")
    ...
    def set_phase_clicked(self):
        try:
            ...
            self.sr830man.ref_phase_shift(False, new_phase)
            self.pushButton_SetPhase.setStyleSheet("background: green")
        except ValueError as ve:
            self.pushButton_SetPhase.setStyleSheet("background: red")
            self.lineEdit_phase.setText(f"{self.sr830man.ref_phase_shift(True):.4f}")

    def sync_current_settings(self):
        if self.sr830man is not None:
            self.comboBox_TimeConstant.setCurrentText(
                SR830Man.time_constant_int2str(self.sr830man.time_constant(True)))
            self.comboBox_Sensitivity.setCurrentText(
                SR830Man.sensitivity_int2str(self.sr830man.sensitivity(True)))
            self.comboBox_FilterSlope.setCurrentIndex(self.sr830man.low_pass_filter_slope(True))
            self.lineEdit_phase.setText(f"{self.sr830man.ref_phase_shift(True):.4f}")
            if self.sr830man.ref_source(True) == 1:
            ...
            self.spinBox_Harmonic.setValue(self.sr830man.detection_harmonic(True))
            self.snap_all_values()

    def conn_changed(self, i: int):
        if self.radioButton_Interf_RS232.isChecked():
            self.sr830man.com_dev = self.comboBox_CONN.currentData()
        elif self.radioButton_Interf_GPIB.isChecked():
            self.sr830man.visa_dev = self.comboBox_CONN.currentText()

    def parity_changed(self, p: str):
        if p == 'None':
            self.sr830man.ser_parity = serial.PARITY_NONE
        elif p == 'ODD':
            self.sr830man.ser_parity = serial.PARITY_ODD
        elif p == 'EVEN':
            self.sr830man.ser_parity = serial.PARITY_EVEN
        else:
    ...
    def refresh_comlist(self):
        ...
        if self.radioButton_Interf_RS232.isChecked():
            ...
            self.comboBox_CONN.addItems(self.sr830man.visa_rm.list_resources())
    ...
    def open_conn(self):
        ...
        try:
            ...
            sr830_logger.info(f"SR830 COM connection opened", extra={"component": "SR830"})
            if self.radioButton_Interf_RS232.isChecked():
                sr830_logger.debug(f"SR830 COM: {self.sr830man.com_dev} "
                                   f"B:{self.sr830man.ser_baudrate} "
                                   f"P:{self.sr830man.ser_parity} "
                                   f"S:{self.sr830man.ser_stopbits}", extra={"component": "SR830"})
            else:
                sr830_logger.debug(f"SR830 Visa addr:: {self.sr830man.visa_dev} ", extra={"component": "SR830"})
        except Exception as e:
            traceback.print_tb(e.__traceback__)
            sr830_logger.error(f"Failed to open SR830 COM connection", extra={"component": "SR830"})
            self.close_conn()
    ...
    def close_conn(self):
        ...
        sr830_logger.info(f"SR830 COM connection closed", extra={"component": "SR830"})
